...
module "redis-cache" {
  ...
  env                           = var.env
  name                          = "${var.product}-cor-frontend-v6"
  business_area                 = "cft"
...
resource "azurerm_key_vault_secret" "redis_connection_string" {
  ...
  value        = "redis://ignore:${urlencode(module.redis-cache-v2.access_key)}@${module.redis-cache-v2.host_name}:${module.redis-cache-v2.redis_port}?tls=true"
  key_vault_id = data.azurerm_key_vault.sscs_key_vault.id
