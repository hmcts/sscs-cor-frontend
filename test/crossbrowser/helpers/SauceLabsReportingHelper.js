...
function updateSauceLabsResult(result, sessionId, jobName) {
  ...
  logger.info(
    `SauceOnDemandSessionID=${sessionId} job-name=${jobName} credentials=${sauceCredentials}`
  );
...
module.exports = function () {
  ...
  event.dispatcher.on(event.test.passed, () => {
    const sessionId =
      container.helpers('WebDriverIO').browser.requestHandler.sessionID;
    const jobName =
      container.helpers('WebDriverIO').config.desiredCapabilities.name;
