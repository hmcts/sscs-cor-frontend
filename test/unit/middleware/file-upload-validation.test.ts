...
describe('#handleFileUploadErrors middleware', () => {
  ...
  it('should catch multer LIMIT_UNEXPECTED_FILE error', () => {
    handleFileUploadErrors(new multer.MulterError('LIMIT_UNEXPECTED_FILE'), req, res, next);
    expect(res.locals.multerError).to.equal(content.en.questionUploadEvidence.error.limitOnlyDocument);
    expect(next).to.have.been.calledOnce.calledWith();
  });
