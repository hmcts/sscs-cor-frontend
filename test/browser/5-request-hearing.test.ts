...
describe.skip('Request a hearing', () => {
  ...
  describe('requesting a hearing page', () => {
    ...
    it('validates that one option must be selected', async () => {
      ...
      expect(await hearingConfirmPage.getElementText('#error-summary-title')).contain(i18n.en.errorSummary.titleText);
      expect(await hearingConfirmPage.getElementText('.govuk-error-summary__body')).contain(i18n.en.hearingConfirm.error.text);
      expect(await hearingConfirmPage.getElementText('#new-hearing-error')).contain(i18n.en.hearingConfirm.error.text);
    });
    ...
    describe('appellant chooses request a hearing', () => {
      ...
      describe('submits the reason', () => {
        ...
        it('shows the hearing booking details', async () => {
          expect(await hearingWhyPage.getHeading()).equal(i18n.en.hearingWhy.booking.header);
        ...
        it('returns the user to the hearing booking page if they sign-in later', async () => {
          await login(page);
          hearingWhyPage.verifyPage();
          expect(await hearingWhyPage.getHeading()).to.equal(i18n.en.hearingWhy.booking.header);
        });
      });
    });
  });
