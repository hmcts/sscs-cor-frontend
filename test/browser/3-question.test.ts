...
describe.skip('Question page', () => {
  ...
  /*before('start services and bootstrap data in CCD/COH', async () => {
    ...
    caseReference = res.ccdCase.id || mockDataHearing.case_reference;
  ...
  it('displays an error message in the summary when you try to submit an empty answer', async () => {
    ...
    expect(await questionPage.getElementText('.govuk-error-summary'))
      .contain(content.en.question.textareaField.error.empty);
    expect(await questionPage.getElementText('#question-field-error'))
      .to.containIgnoreSpaces(content.en.question.textareaField.error.empty);
  });
  ...
  describe.skip('evidence upload per question disabled', () => {
    it('displays guidance for submitting evidence with case reference', async () => {
      ...
      expect(summaryText.trim()).to.equal(content.en.question.sendingEvidence.summary);
  ...
  describe.skip('with evidence upload per question enabled', () => {
    ...
    describe.skip('with javascript OFF', () => {
      ...
      it('display evidence upload section', async () => {
        ...
        expect(headerText).to.equal(content.en.question.evidenceUpload.header);
        expect(addFile).to.equal(content.en.question.evidenceUpload.addFileButton);
      });
      ...
      it('also displays guidance posting evidence with reference', async () => {
        ...
        expect(summaryText).to.contain(content.en.question.evidenceUpload.postEvidence.summary);
        expect(displayedCaseRef).to.equal(caseReference);
      });
      ...
      it('shows the upload evidence page', async () => {
        ...
        expect(await uploadEvidencePage.getHeading()).to.equal(content.en.questionUploadEvidence.header);
      });
      ...
      it('validates that a file has been chosen', async () => {
        ...
        expect(await uploadEvidencePage.getElementText('.govuk-error-summary')).contain(content.en.questionUploadEvidence.error.empty);
        expect(await questionPage.getElementText('#file-upload-1-error')).to.containIgnoreSpaces(content.en.questionUploadEvidence.error.empty);
      });
      ...
      it('Invalid answer text, do not shows question as draft status', async () => {
        ...
        expect(answerState).not.equal(content.en.taskList.answerState.draft.toUpperCase());
      });

      it('shows question as draft status', async () => {
        await questionPage.saveAnswer('Valid Answer');
        const answerState = await taskListPage.getElementText(`#question-${firstQuestionId} .answer-state`);
        expect(answerState).to.equal(content.en.taskList.answerState.draft.toUpperCase());
      });
      ...
      it('tries to upload evidence file that is not an allowed type', async () => {
        ...
        expect(await uploadEvidencePage.getElementText('#file-upload-1-error'))
          .contain(content.en.questionUploadEvidence.error.invalidFileType);
      });
    ...
    describe.skip('with javascript ON', () => {
      ...
      it('displays evidence upload section toggle', async () => {
        ...
        expect(checkboxLabel.trim()).to.equal(content.en.question.evidenceUpload.checkbox);
      });
  ...
  describe.skip('submitting an answer', () => {
    ...
    it('displays question status as completed', async () => {
      const answerState = await taskListPage.getElementText(`#question-${firstQuestionId} .answer-state`);
      expect(answerState).to.equal(content.en.taskList.answerState.completed.toUpperCase());
    });
  });
