...
describe('Hearing Recording request @mya @nightly', function () {
  ...
  it('Navigate to request page', async function () {
  ...
  it('Select hearing recording option and shows list of hearing recording available', async function () {
    ...
    ).to.equal(content.en.hearingRecording.releasedHearingRecordings);
    expect(
      await requestTypePage.getElementText('#outstanding-hearing-recording h3')
    ).to.equal(content.en.hearingRecording.outstandingHearingRecordings);
    expect(
      await requestTypePage.getElementText(
        '#hearing-recording-request-submit-form h3'
      )
    ).to.equal(content.en.hearingRecording.hearingRecordingRequests);
  });
});
