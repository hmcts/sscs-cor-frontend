...
function handleFileUploadErrors(err: any, req: Request, res: Response, next: NextFunction) {
  ...
  if (err instanceof multer.MulterError) {
    if (err.code === 'LIMIT_FILE_SIZE') {
      error = `${i18n.en.questionUploadEvidence.error.tooLarge} ${maxFileSizeInMb}MB.`;
    } else if (err.code === 'LIMIT_FILE_TYPE') {
      error = i18n.en.questionUploadEvidence.error.invalidFileType;
    } else {
      error = i18n.en.questionUploadEvidence.error.fileCannotBeUploaded;
    }
