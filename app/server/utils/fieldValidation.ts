...
function uploadDescriptionValidation(description) {
  ...
  const schema = Joi.string()
    ...
    .options({
      language: {
        any: { empty: `!!${i18n.en.additionalEvidence.evidenceUpload.error.emptyDescription}` },
        string: { max: `!!${i18n.en.hearingWhy.error.maxCharacters}` }
      }
    });
...
function answerValidation(answer, req?) {
  ...
  let emptyErrorMsg = i18n.en.question.textareaField.errorOnSave.empty;

  // On Submit
  if (req.body.submit) {
    emptyErrorMsg = i18n.en.question.textareaField.error.empty;
  }
  ...
  const schema = Joi.string()
    ...
    .options({
      language: {
        ...
        string: { max: `!!${i18n.en.question.textareaField.error.maxCharacters}` }
      }
    });
...
function loginEmailAddressValidation(email) {
  const schema = Joi.string()
    ...
    .options({
      language: {
        any: { empty: `!!${i18n.en.login.emailAddress.error.empty}` },
        string: { email: `!!${i18n.en.login.emailAddress.error.format}` }
      }
    });
...
function tribunalViewAcceptedValidation(acceptView, isConfirm = false) {
  ...
  if (!allowedValues.includes(acceptView)) {
    if (isConfirm) {
      return i18n.en.tribunalView.error.emptyOnConfirm;
    } else {
      return i18n.en.tribunalView.error.emptyOnDecisionPick;
    }
  }
  return false;
}
...
function newHearingAcceptedValidation(newHearing) {
  ...
  if (!allowedValues.includes(newHearing)) {
    return i18n.en.hearingConfirm.error.text;
  }
  return false;
}
