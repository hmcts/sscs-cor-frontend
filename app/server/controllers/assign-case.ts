...
function postIndex(caseService: CaseService, trackYourAppealService: TrackYourApealService) {
  return async (req: Request, res: Response) => {
    ...
    if (!req.body.appealType) {
      return renderError(
        { msg: errorContent('missing', 'appealType'), code: 'missing-appealType' }, req, res
      );
    }
    ...
    if (!tya) {
      return renderError({ msg: errorContent('missing', 'tya'), code: 'tyaNotProvided' }, req, res)
    }
    ...
    const { statusCode, body }: ApiResponse =
      await caseService.assignOnlineHearingsToCitizen(idamEmail, tya, postcode, ibcaReference, req);
    ...
    if (statusCode !== StatusCodes.OK) {
      ...
      const errorField = req.body.appealType === 'ibca' ? 'ibcaReference' : 'postcode';
      return renderError(
        { msg: errorContent('invalid', errorField), code: 'no-matching-record' },
        req,
        res
      );
    }
    ...
    const { appeal } = await trackYourAppealService.getAppeal(req.session.case.case_id, req);
    req.session.appeal = appeal;
    req.session.case.case_reference = req.session.case.case_id ? req.session.case.case_id.toString() : '';
...
function validateField(reqBody: any, field: string) {
  ...
  return errorType ?
    { msg: errorContent(errorType, field), code: `${errorType}-${field}` } : false;
}

const errorContent = (errType, field) => content[i18next.language].assignCase.errors[errType][field];
