...
function getHearing(req: Request, res: Response) {
  if (!isFeatureEnabled(Feature.MANAGE_YOUR_APPEAL, req.cookies) || req.session.appeal.hearingType === 'cor') return res.render('errors/404.html', { ft_welsh: req.session.featureToggles.ft_welsh });
  ...
  return res.render('hearing-tab.html', { hearingInfo, attending, hearingArrangements, ft_welsh: req.session.featureToggles.ft_welsh });
}
