...
function getSubmitQuestion(questionService: QuestionService) {
  return (req: Request, res: Response) => {
    ...
    res.render('submit-question.html', { questionOrdinal, ft_welsh: req.session.featureToggles.ft_welsh });
  };
}
...
function postSubmitAnswer(questionService: QuestionService) {
  return async(req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const response = await questionService.getAllQuestions(hearingId,req);
