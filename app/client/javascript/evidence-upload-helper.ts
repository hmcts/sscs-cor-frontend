export class EvidenceUploadHelper {
  ...
  setFileUploadState(): void {
    document.getElementById(this.FILE_UPLOAD_ID).className =
      'file-display-none';
    const fileUploadLabel: HTMLElement = document.querySelector(
      this.FILE_UPLOAD_LABEL_SELECTOR
    );
  ...
  setRevealStartState(): void {
    const uploadedFiles = document.querySelectorAll(
      '#files-uploaded tr.evidence'
    );
    const uploadError = document.querySelectorAll('#file-upload-1-error');
    const provideEvidence = document.getElementById(
      this.CHECKBOX_ID
    ) as HTMLInputElement;
  ...
  showHideElements(
    noJsElementSelector: string,
    jsElementSelector: string
  ): void {
    const noJsElements: NodeListOf<HTMLElement> =
      document.querySelectorAll(noJsElementSelector);
    const jsElements: NodeListOf<HTMLElement> =
      document.querySelectorAll(jsElementSelector);
